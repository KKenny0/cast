...
漫画模具用日式单色漫画的视觉语言讲故事。不是加了漫画边框的排版，而是用分格节奏、墨色对比、集中线和留白来制造戏剧性。它服从 Quiet Paper：暖纸底、深墨文字、细线分格、低噪纹理，不使用霓虹色和大面积纯黑纯白。力量来自墨色的浓淡和构图的紧张感。
...
| **三浦建太郎 — 暗黑压迫** | 大面积深墨、强对比、密集纹理、压迫感强 | 冲突/困境/黑暗面/挣扎、情绪强烈 | `--bg: #F0F0F0; --tone: #C0C0C0` |
...
**所有元素用 CSS + SVG 实现。暖纸与深墨为主，低对比网点为辅，不用装饰性彩色。**
...
- **单色调性**：`filter: grayscale(100%) contrast(1.08);` 保持漫画单色纸面感。如原图本身是单色/线条风格，可保留原色
...
**核心约束**：
- 配色限于暖纸(`--paper/--panel`)、深墨(`--ink`)、灰墨(`--gray-*`)三值
- 如需强调，用墨色、字号和位置解决，不用彩色
- 边框统一 1px hairline，必要时用网点或留白制造分格节奏
...
写入：
- 单张：`<system_temp>/card_comic_{name}.html`
- 多张：`<system_temp>/card_comic_{name}_{N}.html`（N = 01, 02, ...）

## 步骤 5：自检

- [ ] 一眼看上去像漫画页面吗？如果看着像"加了边框的普通排版"，重做
- [ ] 有没有至少 3 个分格？漫画没有分格就不是漫画
- [ ] 单色层次是否清楚？有没有靠分格节奏制造戏剧性？
- [ ] 有没有至少 1 个漫画特有元素（集中线/速度线/对话泡/效果字/网点灰）？
- [ ] 格子大小是否有对比？（一个大格 + 几个小格，不是全部等大）
- [ ] 是否避免了装饰性彩色？灰色只用于网点和弱层次
- [ ] 正文字号 ≥ 36px？标题 ≥ 72px？
- [ ] 有没有一个格子让人第一眼被吸进去？
- [ ] 是否避免了等分、对称、均匀间距？
...
单张：
```bash
node assets/capture4k.js <system_temp>/card_comic_{name}.html ~/Downloads/{name}.png 1080 800 2 fullpage
```

多张：逐张截图，文件名带序号
```bash
node assets/capture4k.js <system_temp>/card_comic_{name}_01.html ~/Downloads/{name}_01.png 1080 800 2 fullpage
node assets/capture4k.js <system_temp>/card_comic_{name}_02.html ~/Downloads/{name}_02.png 1080 800 2 fullpage
# ... 逐张执行
```
